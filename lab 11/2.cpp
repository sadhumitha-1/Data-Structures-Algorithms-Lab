...
public:
    ...
    bool insert(int key) {
        ...
        while (i < size) {
            ...
            int position = (index + i*i) % size;
            
            // Found an empty slot or a deleted slot
            if (!table[position].isOccupied || table[position].isDeleted) {
                table[position].key = key;
                table[position].isOccupied = true;
                table[position].isDeleted = false;
                count++;
                return true;
            }
            // Key already exists
            else if (table[position].key == key && !table[position].isDeleted) {
